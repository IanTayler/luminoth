...
def focal_loss(cls_scores, cls_probs, targets, num_classes, gamma=2.0,
    ...
    with tf.name_scope('focal_loss'):
        if weights is None:
            weights = 1.
        targets_one_hot = tf.one_hot(targets, depth=num_classes + 1)
        ...
        # Get the probability assigned to the target label in each case.
        # `tf.reduce_max` could be swapped with `tf.reduce_sum` or
        # `tf.reduce_min` because we should only get one non-zero value for
        # each row.
        true_probs = tf.reduce_max(
            tf.multiply(targets_one_hot, cls_probs, name='get_true_probs'),
            axis=1, name='reduce_probs'
        )
        focal_weights = tf.pow(1. - true_probs, gamma, name='power_gamma')
